...
description: >-
  ...
  over Qt6 C++ code for the Automotive UI Showcase project —
  or suggest before committing. Runs deterministic linting
  (60+ rules) then six parallel deep-analysis agents covering
  model contracts, ownership, threading, API correctness,
  error handling, and performance. Applies a project-specific
  filter that deeply analyses memory leaks and thread conflicts
  in QTimer and QSerialPort code (SimulatorService and
  SerialService). Reports only high-confidence issues
...
- QTimer created with `new` and not parented/destroyed when the
  owning class is destroyed (the SimulatorService mock pattern:
  ensure the timer has a parent or an explicit `deleteLater` on
  teardown)
- QSerialPort `readyRead` handler accessing `QSerialPort` state
  outside the GUI thread (the SerialService runs the QSerialPort
  on a non-GUI thread by default — confirm the read thread
  matches the access thread or uses queued signals)
- QSerialPort object lifetime races when the port is closed
...
These rules are mandatory for this project and supplement the generic
Qt C++ review process above.

### Custom Rule 1 — Deep filter: QTimer & QSerialPort memory leaks and thread conflicts

For this project, every review MUST perform extra-deep analysis of memory
leaks and thread-safety conflicts in code that touches `QTimer` or
`QSerialPort`. These are the two hardware-simulation/communication hotspots:
...
- Any `QTimer`/`QSerialPort` created with `new` must have a `QObject` parent,
  live in a smart pointer, or be explicitly deleted/`deleteLater()`-ed.
- `QSerialPort` must be `close()`-ed on teardown and on error; a re-opened
  port must not leak the previous connection or leave dangling `connect()`s.
- `connect()` calls to timer/serial signals must not accumulate on repeated
  start/stop cycles (duplicate connections leak slots and fire multiple times).
- Frame buffers accumulating serial bytes must be bounded and cleared on
  parse — an unbounded `QByteArray` growth is a leak.

**Thread-conflict checks (raise confidence and priority for these):**
- The `QTimer` must be started/stopped and its `timeout` handled on its
  owning thread only.
- `QSerialPort` I/O and the ViewModel update it feeds must not cross threads
  without a queued connection; flag any `Qt::DirectConnection` from a serial
  read thread to a GUI-thread ViewModel.
- Shared telemetry state written by the serial/timer path and read by QML
  bindings must be synchronized or confined to a single thread.

Findings in this category should be reported even at confidence 80–89, and
their mitigation must name the exact ownership/threading fix.

### Custom Rule 2 — Source-swap invariance (MVVM contract)

`SimulatorService` and `SerialService` must feed the *same* ViewModel
interface (`Q_PROPERTY` + `NOTIFY`, `Q_INVOKABLE`). Flag any code that leaks
the concrete service type into the ViewModel/QML contract, since switching
simulation ↔ hardware must require zero QML changes (AGENT.md §3).
