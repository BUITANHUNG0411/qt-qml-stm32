...
description: >-
  ...
  over Qt6 QML code for the Automotive UI Showcase project --
  or suggest before committing. Runs deterministic linting
  (47+ rules, including JS-in-QML ban), then six parallel
  deep-analysis agents covering bindings, layout, loaders,
  delegates, states, and performance. Optionally invokes
  system qmllint for type-level checks. Reports only
  high-confidence issues (>80/100) with structured
  mitigations. Read-only -- never modifies code. Enforces the
  project's absolute Zero JavaScript in QML hard rule.
...
These rules are mandatory for this project and override or supplement
the generic QML review process above.

### Custom Rule 1 — Zero JavaScript in QML (HARD, non-negotiable)

This is the single highest-priority review rule. Before checking any other
category, Phase 1 MUST flag every occurrence of JavaScript logic in `.qml`
files as a **CRITICAL FAILURE** at confidence **100/100**.

**Forbidden patterns** (any of these in a `.qml` file = finding):
- `function`, `if`, `for`, `while`, `switch`, `var`, `let`, `const`
- Any multi-statement JS block or conditional expression used for control flow
- `Qt.createComponent(url)` with a dynamic JS string argument

**Allowed in `.qml`** (do NOT flag these):
- Declarative property bindings: `prop: expression`
- `Q_INVOKABLE` method calls for user input forwarding
- Declarative `States`, `Transitions`, `Behavior`, `NumberAnimation`
- Simple single-expression property bindings even if they look JS-like
  (e.g., `visible: someProperty > 0` is a binding, not logic)

**Agent missions update**: All six agents must also flag any JS logic
they discover. This rule is checked in every agent, not just Agent 6.

**Mitigation for violations**: All logic must be moved to a C++ ViewModel
exposed via `Q_PROPERTY`/`NOTIFY` and `Q_INVOKABLE`. QML must remain a
dumb view — rendering, animation, and typography only.

### Custom Rule 2 — MVVM source-swap invariance

When reviewing, verify that no QML file depends on whether the backend is
`SimulatorService` or `SerialService`. QML must consume the ViewModel
interface only (`Q_PROPERTY` + `Q_INVOKABLE`); switching from QTimer mock
to QSerialPort hardware must require zero QML changes.
...
- **JS_BAN** (Zero-JS Enforcement) -- GATEKEEPER: any `function`, `if`,
  `for`, `while`, `switch`, `var`, `let`, `const`, or inline JavaScript
  block in a `.qml` file is a CRITICAL FAILURE (confidence: 100/100,
  no exceptions). This is the project's hardest rule (AGENT.md §1).
  The linter MUST flag these first and highest.
