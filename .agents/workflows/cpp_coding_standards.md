...
description: >-
  ...
  files in the Automotive UI Showcase (Qt 6 / QML + STM32) project — ViewModel
  classes (`Q_PROPERTY`/`Q_INVOKABLE`), Qt Quick backends, hardware service
  layers (`QTimer`/`QSerialPort`), plain C++ utilities, structs, free-function
  headers, and entry points like main.cpp. Documents the MVVM architecture,
  thread-safety of the SimulatorService/SerialService services, QML exposure,
  and UART frame handling toward the STM32F103C8T6. Triggers on: "document
  this class", "write docs for my C++", "document main.cpp", "C++ API docs",
  "document my Qt app", or whenever C++ or header files are provided and
  documentation is needed. Works with single files, pasted code, or entire
  project folders. DO NOT use if the user asks for QDoc format output.
...
These rules apply when documenting C++ code in this project.

### Custom Rule 1 — Document the MVVM ViewModel contract

Every `Q_PROPERTY` and `Q_INVOKABLE` on a ViewModel class must document:
- The **vehicle mode** it feeds (Bike / Scooter / HMI / Digital Car)
- The **property binding flow**: which QML screen reads it and what
  property name it exposes to QML
- The **thread of operation** (all ViewModel properties must be updated on
  the GUI thread, even when the underlying service runs elsewhere)

### Custom Rule 2 — Document the SimulatorService / SerialService duality

`SimulatorService` and `SerialService` are the two concrete implementations
of the same ViewModel interface. Document each service class with:
- Which `QTimer`-driven ticks or `QSerialPort` `readyRead` events update
  which ViewModel properties
- The **thread affinity** of the service (SimulatorService lives on the GUI
  thread; SerialService must handle UART I/O off-thread with queued signals)
- The **UART frame format** expected by SerialService: length-prefixed
  `\n`-terminated ASCII, 115200 8N1, toward an STM32F103C8T6

Explicitly note in each service's documentation that the corresponding QML
layer must remain unchanged between the two — this is the project's
source-swap invariance rule (AGENT.md §3).

### Custom Rule 3 — Memory and lifecycle annotations

For every `QTimer` or `QSerialPort` created with `new`:
- State the **owner and parent** in the Ownership & Lifecycle section
- Describe the **teardown sequence** (`deleteLater`, `close()`, etc.)
- Note any **re-entrancy guards** needed for callbacks that emit signals
