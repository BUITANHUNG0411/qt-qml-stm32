...
description: >-
  ...
  for the Automotive UI Showcase (Qt 6 / QML + STM32). Use whenever QML code
  is the primary subject: writing, reviewing, fixing, refactoring, optimizing,
  or debugging QML files, components, or bindings. Enforces the project's
  absolute "Zero JavaScript in QML" rule -- only Property Binding and
  Q_INVOKABLE calls are permitted in .qml files. Do NOT trigger for purely
  conversational QML questions where no code is produced or examined.
...
These rules are mandatory for this project and take precedence over the
generic QML guidance above where they are stricter.

### Rule 1 — Absolute ban on JavaScript logic in .qml files

No JavaScript logic may be written inside any `.qml` file. This is a HARD,
non-negotiable project rule (see AGENT.md §1).

- **Forbidden** in `.qml`: `function`, `if`, `for`, `while`, `switch`, `var`,
  `let`, `const`, inline JS blocks, or any imperative script logic.
- **Allowed** in `.qml`:
  - Declarative **Property Bindings** (`prop: expression`) for all rendering,
    layout, and state-derived values.
  - `Q_INVOKABLE` calls into C++ for user input and actions.
  - Declarative `States`, `Transitions`, `Behavior`, and `NumberAnimation` for
    animation and layout adaptation between vehicle modes (Bike → Scooter →
    HMI → Digital Car).
- **All logic lives in C++** — ViewModels expose state via `Q_PROPERTY` +
  `NOTIFY` signals; Services expose behaviour via `Q_INVOKABLE` / slots.
- **Before finishing any QML work**, grep the file for the JS keywords above
  and fail the change if any logic block exists. QML is a *dumb view*.

### Rule 2 — QML is a view only

QML renders, animates, and typesets only. It must not hold business state,
generate telemetry, or parse serial frames. The same QML must work unchanged
when the backend is swapped between `SimulatorService` (QTimer mock) and
`SerialService` (QSerialPort) — source-swap invariance (AGENT.md §3).
