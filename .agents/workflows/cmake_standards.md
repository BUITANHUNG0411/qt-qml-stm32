...
description: >-
  ...
  the Automotive UI Showcase (Qt 6 / QML + STM32). Ensures build
  configurations are optimised for the local Linux Mint development
  environment and links the required modules (Core, Gui, Qml, Quick,
  SerialPort).
...
These rules are mandatory for this project and supplement the hard rules above.

### Custom Rule 1 — Optimise builds for the local Linux Mint dev environment

The primary build target is a developer PC running Linux Mint (Ubuntu-based,
GCC/Ninja toolchain). Generate CMake that builds cleanly and efficiently there:

- **Default to the Ninja generator** when configuring
  (`cmake -G Ninja -S . -B build`) — it is the fastest common generator on
  Linux Mint and is what the project's configure step assumes.
- **Set a sensible default build type.** Because single-config generators
  (Make/Ninja) leave `CMAKE_BUILD_TYPE` empty by default, guard it so local
  builds are optimised:
  ```
  if(NOT CMAKE_BUILD_TYPE AND NOT CMAKE_CONFIGURATION_TYPES)
      set(CMAKE_BUILD_TYPE Debug CACHE STRING "Build type" FORCE)
  endif()
  ```
  Use `Debug` for day-to-day development and document that `Release`
  (`-DCMAKE_BUILD_TYPE=Release`) is used for the final showcase build.
- **Export compile commands** for the local clangd/IDE tooling:
  `set(CMAKE_EXPORT_COMPILE_COMMANDS ON)`.
- **Find Qt on Linux Mint** without hardcoding Windows paths. Rely on
  `CMAKE_PREFIX_PATH` / the `Qt6_DIR` environment variable rather than
  absolute `C:/Qt/...` paths. Never emit Windows-only or MSVC-only settings.
- **Link the modules this project actually uses** and no more:
  `find_package(Qt6 6.8 REQUIRED COMPONENTS Core Gui Qml Quick SerialPort)`
  and `target_link_libraries(<target> PRIVATE Qt6::Core Qt6::Gui Qt6::Qml
  Qt6::Quick Qt6::SerialPort)`. `SerialPort` is required for the future
  `SerialService` (UART to the STM32F103C8T6).
- **Enable warnings for local GCC/Clang** without breaking the build:
  add `-Wall -Wextra` via a generator expression scoped to GNU/Clang so the
  config stays portable
  (`$<$<CXX_COMPILER_ID:GNU,Clang>:-Wall;-Wextra>`).
- **Keep the build tree out of the source tree** (`build/`) so the local
  Linux Mint workflow stays clean; never commit generated artefacts.

Keep these as project defaults — do not add Windows/macOS-specific build
logic unless the user explicitly asks, since the local target is Linux Mint.

### Custom Rule 2 — QML via qt_add_qml_module only

Per the project's Zero-JS / MVVM architecture, all QML screens and components
must be registered through `qt_add_qml_module()` (never a hand-written `.qrc`),
so the QML module system handles type registration for the C++ ViewModels
exposed to QML.
